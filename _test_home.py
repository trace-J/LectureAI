...
is imported, so no test can read or write the real ~/.lectureai.

    from _test_home import fresh_home
    HOME = fresh_home()            # must run before `from lectureai import config`
"""
import os
import tempfile
...
def fresh_home(schedule: str | None = SAMPLE_SCHEDULE) -> Path:
    """Create an empty home, point $LECTUREAI_HOME at it, seed the schedule."""
    home = Path(tempfile.mkdtemp(prefix="lectureai-test-"))
    os.environ["LECTUREAI_HOME"] = str(home)
